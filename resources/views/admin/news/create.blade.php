        ...
        <form action="{{ route('admin.news.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2" id="event_date_wrapper" style="display: {{ old('type') === 'event' ? 'block' : 'none' }}">
                    ...
                    <input type="date" name="event_date" id="event_date" value="{{ old('event_date') }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-colors [color-scheme:dark]">
                    @error('event_date')
                        <p class="text-rose-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="space-y-2 pt-2">
                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="is_published" value="1" {{ old('is_published', true) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-teal-500 rounded bg-slate-900 border-slate-700 focus:ring-teal-500 focus:ring-offset-slate-900">
                    <span class="text-slate-300 font-medium">Publish Immediately</span>
                </label>
            </div>
            ...
            <div class="pt-6 border-t border-slate-700/50 flex items-center justify-end space-x-4">
                ...
                <x-public.btn type="submit" variant="primary">Save</x-public.btn>
            </div>
        </form>
    </div>
</div>
