                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end gap-2">
                            <form action="{{ route('admin.faqs.toggle-status', $faq) }}" method="POST" class="inline-block" title="Toggle Status">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none {{ $faq->is_active ? 'bg-teal-500' : 'bg-slate-600' }}">
                                    <span class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $faq->is_active ? 'translate-x-5' : 'translate-x-0' }}"></span>
                                </button>
                            </form>
                            
                            <button type="button" onclick="editFaq({{ json_encode($faq) }})" class="p-2 inline-flex items-center justify-center rounded-lg text-slate-400 hover:text-teal-400 hover:bg-slate-700/50 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-600" title="Edit">
