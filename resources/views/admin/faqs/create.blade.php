...
@section('title', 'Add FAQ - Admin Panel')
    ...
    <div class="bg-slate-800/50 rounded-2xl border border-slate-700/50 backdrop-blur-sm p-6 md:p-8">
        <form action="{{ route('admin.faqs.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2 flex flex-col justify-center pt-2">
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-teal-500 rounded bg-slate-900 border-slate-700 focus:ring-teal-500 focus:ring-offset-slate-900">
            ...
            <div class="pt-6 border-t border-slate-700/50 flex items-center justify-end space-x-4">
                ...
                <x-public.btn type="submit" variant="primary">Save FAQ</x-public.btn>
            </div>
        </form>
    </div>
</div>
